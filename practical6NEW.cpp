...
display whole  dkeyword sorted  in  ascending/  Descending  order.  Also  find  how  
many  maximum comparisons may require for finding any keyword. Use Binary Search Tree for 
implementation.
*/

#include<iostream>
#include<string.h>   //cstring also works
using namespace std;
...
void dict :: create()
{
    ...
    while(ch == 1);   //here can also be

    }

void dict ::  insert(node * root,node *temp)
{
    if(strcmp (temp->k, root->k) < 0 ) //has porpose if small go to left or else right
    //if(root != NULL)
    {
        if(root->left == NULL)
        root->left = temp;
...
void dict:: disp(node * root)
{
    if( root != NULL)
    {
        disp(root->left);             // watchout it will display root too 
        cout<<"\n Key Word :"<<root->k;
        cout<<"\t Meaning :"<<root->m;
        disp(root->right);
    }
...
int ch;
dict d;
d.root = NULL;   //withowt constructor NULL
    ...
    switch(ch)
    {
        ...
        {
            d.disp(d.root);   //dont change the position of root
        }
        break;
